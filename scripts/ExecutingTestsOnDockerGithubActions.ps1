...
docker exec -u root ocaramba_selenium bash -c 'apt-get update && apt-get install -y zip'
docker exec -u ocaramba ocaramba_selenium bash -c 'mkdir -p /tmp/TestOutput && ls ./Ocaramba.Tests.NUnit/bin/Debug/net8.0/TestOutput/ && cp -r ./Ocaramba.Tests.NUnit/bin/Debug/net8.0/TestOutput/. /tmp/TestOutput &&  cd /tmp && zip -r TestOutput.zip TestOutput'
